...
describe( `GET /locales`, () => {
  it('Return an Array of Strings', async () => request(app())
    ...
    .expect(200)
    .expect(function(res) {
      expect(Array.isArray(res.body.locales)).toBeTruthy();
      res.body.locales.forEach((str) => expect(typeof str).toBe('string'));
    })
  );
  ...
  it('Return at least one locale name', async () => request(app())
    ...
    .expect(function(res) {
      ...
      expect(res.body.locales.length).toBeGreaterThanOrEqual(1);
    })
  );
  ...
  it('Names of locales are written in a standardized locale naming form', async () => request(app())
    ...
    .expect(200)
    .expect(function(res) {
      res.body.locales.forEach((str) => expect(/^[a-z]{2}(?:-[A-Z]{2})?$/.test(str)).toBeTruthy());
    })
  );
});
