...
describe(`CalendarsService - Month`, () => {
  ...
  it('Get a specific month within a civil year: 2025 January', async () => {
    const data = service.getMonth({ year: 2025, month: 0 });
    const firstDate = moment(data.celebrations[0].date);
    const lastDate = moment(data.celebrations[data.celebrations.length - 1].date);

    expect(formatDate(firstDate)).toBe('2025-01-01');
    expect(formatDate(lastDate)).toBe('2025-01-31');
  });
